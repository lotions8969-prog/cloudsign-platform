      ...
      setAuditLogs([
        {
          ...
          actorEmail: "demo@allcontract.com",
          details: { note: "デモデータ" },
          timestamp: { toDate: () => new Date(), toMillis: () => Date.now() } as any,
          currentLogHash: "demo-hash",
        ...
        <div className="card p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              ...
              {envelope.description && <p className="text-gray-500 text-sm">{envelope.description}</p>}
              <p className="text-xs text-gray-400 mt-2">作成日時: {formatTimestamp(envelope.createdAt)}</p>
            </div>
            <div className="flex gap-2 flex-shrink-0">
              {envelope.status === "completed" && envelope.signedPdfUrl && envelope.signedPdfUrl !== "#" && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="card p-6">
              ...
              <dl className="space-y-3">
                ...
                <InfoRow label="金額" value={envelope.ebookkeepingIndex?.amount?.toLocaleString("ja-JP", { style: "currency", currency: "JPY" })} />
            ...
            <div className="card p-6">
              ...
              {envelope.signatureInfo ? (
                <dl className="space-y-3">
                  ...
                  <InfoRow label="PDFハッシュ (SHA-256)" value={`${envelope.signatureInfo.pdfSha256Hash?.substring(0, 16)}...`} />
                  <InfoRow label="TSAタイムスタンプ" value={envelope.signatureInfo.timestampAuthority ?? "なし"} />
                </dl>
              ) : (
            ...
            <div className="card p-6 md:col-span-2">
              ...
              <div className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="font-mono text-xs text-gray-700 break-all">{envelope.originalPdfSha256}</p>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="card divide-y divide-gray-100">
            ...
            {auditLogs.map((log) => (
              <div key={log.id} className="p-4">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <span className="font-medium text-gray-800 text-sm">{getActionLabel(log.action)}</span>
                    {log.actorEmail && <span className="text-gray-500 text-xs ml-2">by {log.actorEmail}</span>}
                  </div>
                  <p className="text-xs text-gray-400 flex-shrink-0">{formatTimestamp(log.timestamp)}</p>
                </div>
              </div>
            ))}
          </div>
